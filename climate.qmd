...
```{python}
info_df, maca_da = process_maca({'buffalo': buffalo_gdf}, scenarios=['pr'], climates=['rcp85', 'rcp45'], years=[2006,2035])
info_df
```
...
plot_das(maca_da[0][[0,10,20,29]],
    titles=maca_da[0][[0,10,20,29]].coords['time'].values)
```
...
plot_das(maca_da[1][[0,10,20,29]],
    titles=maca_da[1][[0,10,20,29]].coords['time'].values)
```
...
```{python}
info2_df, maca2_da = process_maca({'buffalo': buffalo_gdf}, scenarios=['pr'], climates=['rcp85', 'rcp45'], years=[2036,2065])
info_df
```
...
```{python}
buffalo_sand_da = buffalo_da.rio.reproject_match(slope_da).rio.clip(buffalo_gdf.geometry)
maca85_da = maca_da[0].rio.reproject_match(slope_da).rio.clip(buffalo_gdf.geometry)
maca45_da = maca_da[1].rio.reproject_match(slope_da).rio.clip(buffalo_gdf.geometry)
slope_da_clip = slope_da.rio.clip(buffalo_gdf.geometry)
```

```{python}
maca852_da = maca2_da[0].rio.reproject_match(slope_da).rio.clip(buffalo_gdf.geometry)
maca452_da = maca2_da[1].rio.reproject_match(slope_da).rio.clip(buffalo_gdf.geometry)
```
...
```{python}
buffalo_sand_da.plot()
```

```{python}
#| label: fig-maca85time
from landmapy.plot import plot_das

plot_das(maca85_da[[0,10,20,29]],
    titles=maca_da[0][[0,10,20,29]].coords['time'].values)
```

```{python}
#| label: fig-maca45time
from landmapy.plot import plot_das

plot_das(maca45_da[[0,10,20,29]],
    titles=maca_da[1][[0,10,20,29]].coords['time'].values)
```
...
maca_now_da = xr.concat([maca45_da.mean(dim='time'),
    (maca85_da.mean(dim='time') - maca45_da.mean(dim='time')),
    maca85_da.mean(dim='time')], dim='rcp')
maca_now_da = maca_now_da.assign_coords(rcp=["RCP45","diff","RCP85"])
maca_fut_da = xr.concat([maca452_da.mean(dim='time'),
    (maca852_da.mean(dim='time') - maca452_da.mean(dim='time')),
    maca85_da.mean(dim='time')], dim='rcp')
maca_fut_da = maca_fut_da.assign_coords(rcp=["RCP45","diff","RCP85"])
```
